...
// 修订历史：
//     初始版本。
//     修改了共享内存的累加过程，用多步规约实现。
//     修改了核函数，修改了 block 的分配方式，更正了一些不符合编码规范的地方。
//     规范了编码，减少了核函数中共享内存的使用。
//     修改了输出图像 ROI 子区域设置的方法，更正了核函数中对越界点处理时的错误。
//     调用 copyToCurrentDevice 函数实现图像从 Host 端到 Device 端的拷贝。
//     并更改代码规范问题，合并 In-Place 和 Out-Place 版本的核函数。
//     使用金字塔规约操作完成像素点平均值的计算。
//     取消了对马赛克块大小 mossize < 32 的限制。修改了 block 的分配方式。
//     核函数中取消了线程返回，修改为依次做判断。
