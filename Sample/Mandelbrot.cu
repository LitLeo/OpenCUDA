...
// Kernel 函数：_mandelbrotKer（Mandelbrot集生成）
// 以显示区域 W 内所有的点(mc, mr)作为初始迭代值 p = mc + mri 对 z = z * z + p
// 进行 times 次迭代，z 的初始值为 0。迭代完成后比较 z 的模和逃逸半径 radius
// 的大小，根据比较结果进行着色。 
...
// Kernel 函数: _binarizeInKer（Mandelbrot集生成）
static __global__ void _mandelbrotKer(ImageCuda outimg, Mandelbrot param)
{
...
// Host 成员方法：mandelbrot（Mandelbrot集生成）
__host__ int Mandelbrot::mandelbrot(Image *outimg)
{
    ...
    // 调用核函数
    _mandelbrotKer<<<gridsize, blocksize, colorCount + 1>>>(subimgCud, *this);
...
// Host 成员方法：mandelbrotHost（Mandelbrot 集生成）
__host__ int Mandelbrot::mandelbrotHost(Image *outimg)
{
    // 检查图像是否为 NULL，如果为 NULL 直接报错返回。
    if (outimg == NULL)
        return NULL_POINTER;
    ...
    int colorcnt = this->getColorCount();
    int exp = this->getExponent();
    float fromc = this->getScopeFrom().getReal();
    float fromr = this->getScopeFrom().getImaginary();
    float toc = this->getScopeTo().getReal();
    float tor = this->getScopeTo().getImaginary();

    // 计算各个颜色阶的颜色值。
    unsigned char *colorbuf = new unsigned char [colorcnt + 1];
    for (int i = 0; i <= colorcnt; i++) {
        colorbuf[i] = (unsigned char)((256 * (colorcnt - i) /
                                       colorcnt)/* & 0xFF*/);
    }

    for (int r = 0; r < subimgCud.imgMeta.height; r++) {
        for (int c = 0; c < subimgCud.imgMeta.width; c++) {

            // 将坐标的原点定位到图像中心，将图像的范围设定为 -1.5 到 1.5
            float mc = (toc - fromc) * c / subimgCud.imgMeta.width + fromc;
            float mr = (tor - fromr) * r / subimgCud.imgMeta.height + fromr;
