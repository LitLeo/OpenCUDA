...
//
// 生成 Julia 集
// 功能说明：以显示区域 W 内所有的点 (mc, mr) 作为初始迭代值 z = z * z + p 进行
//           times 次迭代，z 的初始值为 mc + mr。迭代完成后比较 z 的模和逃逸半
//           径 radius 的大小，根据比较结果进行着色。
...
public:
    ...
    Julia()
    {
        ...
        this->constP = Complex(-0.8f, 0.156f);  // 常量 p 为 -0.8 + 0.16i
    ...
    // 成员方法：getConstP（获取常量 p）
    // 获取成员变量 constP 的值。
    __host__ __device__ Complex  // 返回值：返回 constP 成员变量的值
    getConstP() const
    {
        // 返回 constP 成员变量的值
        return this->constP;
    ...
    // Host 成员方法：julia（Julia 集的生成）
    // 以显示区域 W 内所有的点 (mc, mr) 作为初始迭代值 z = z * z + p 进行 times
    // 次迭代，z 的初始值为 mc + mr。迭代完成后比较 z 的模和逃逸半径 radius 的
    // 大小，根据比较结果进行着色。
    ...
    // Host 成员方法：juliaHost（Julia 集的生成）
    // 以显示区域 W 内所有的点 (mc, mr) 作为初始迭代值 z = z * z + p 进行 times
    // 次迭代，z 的初始值为 mc + mr。迭代完成后比较 z 的模和逃逸半径 radius 的
    // 大小，根据比较结果进行着色。该函数在 Host 端由 CPU 串行实现。
    __host__ int           // 返回值：函数是否正确执行，若函数正确执行，返回
                           // NO_ERROR。
    juliaHost(
            Image *outimg  // 输出图像
